...
function initWebGLBackground() {
  try {
    ...
    // Mouse movement effect - only apply in hero section
    let mouseX = 0;
    let mouseY = 0;

    document.addEventListener('mousemove', (event) => {
      // Only track mouse position if we're hovering over the hero section
    ...
    const animate = () => {
      ...
      // Respond to mouse - only in hero section
    ...
    // Remove the container if there was an error
